 ...
 * 从火山引擎结果生成字级别字幕
 *
 * 用法: node generate_subtitles.js <volcengine_result.json> [delete_segments.json] [output_dir]
...
if (deleteFile && deleteFile.length > 0 && fs.existsSync(deleteFile) && fs.statSync(deleteFile).isFile()) {
...
const outputPath = path.join(outputDir, 'subtitles_words.json');
fs.writeFileSync(outputPath, JSON.stringify(wordsWithGaps, null, 2));
